...
router.get("/", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to fetch data_produk" });
  }
});
...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (rowIndex === -1) return res.status(404).json({ error: "Produk not found" });
    ...
    console.error("PUT produk error:", err);
